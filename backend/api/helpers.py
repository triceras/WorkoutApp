...
import time
from .models import Exercise, YouTubeVideo, WorkoutPlan  # Updated to include both models
...
def get_video_id(exercise_name):
    """Get or fetch YouTube video ID for exercise."""
    try:
        standard_name = standardize_exercise_name(exercise_name)
        cache_key = f'exercise_video_{standard_name}'
        
        # Try cache first
        cached_data = cache.get(cache_key)
        if cached_data:
            logger.info(f"Using cached video data for '{standard_name}'")
            return cached_data
        ...
        if video and video.video_id:
            logger.info(f"Found video data in database for '{standard_name}'")
            video_data = {
        ...
        # Search YouTube
        logger.info(f"Searching YouTube for video of '{standard_name}'")
        video_data = youtube_service.search_exercise_video(standard_name)
        ...
        logger.error(f"Error getting video data for '{exercise_name}': {str(e)}")
        return None

def standardize_exercise_name(exercise_name):
    """Standardize exercise name for database lookups."""
    ...
    # Check mapping first
    if name in EXERCISE_NAME_MAPPING:
        return EXERCISE_NAME_MAPPING[name]
...
#     """
#     Fetches YouTube video details from the YouTube Data API.
#     Adds "workout" to the query and only returns the first result.
#     """
#     url = "https://www.googleapis.com/youtube/v3/search"
#     # Add 'workout' to the query
#     params = {
#         'part': 'snippet',
#         'q': query + ' workout',  # Added 'workout' to query
#         'key': YOUTUBE_API_KEY,
...
#         logger.error(f"Error fetching YouTube video data for query '{query}': {e}")
#         return None
...
def get_youtube_video(exercise_name):
    """
    Get YouTube video for an exercise by checking cache, database, and YouTube API.
    """
    try:
        standard_name = standardize_exercise_name(exercise_name)
        ...
        # Query YouTube API as last resort
        try:
            video_data = youtube_service.search_exercise_video(standard_name)
            if video_data:
                # Save to database
                ...
                # Cache the result
                cache.set(cache_key, video_data, timeout=86400)  # 24 hour cache
                return video_data
                
        except Exception as e:
            logger.error(f"Error fetching video from YouTube API: {str(e)}")
            return None

    except Exception as e:
        logger.error(f"Error in get_youtube_video for '{exercise_name}': {str(e)}")
        return None
...
def assign_video_ids_to_exercises(workout_plan_data):
    ...
    Creates Exercise objects in the database if they don't exist.
    """
    ...
    for day in workout_plan_data.get('workoutDays', []):
        if day.get('type') in ['workout', 'active_recovery']:
            ...
            for exercise in exercises:
                ...
                try:
                    existing_exercise = Exercise.objects.get(name__iexact=exercise_name)
                    logger.info(f"Exercise '{exercise_name}' found in database.")
                except Exercise.DoesNotExist:
                    # Create a new exercise object
                    logger.info(f"Exercise '{exercise_name}' not found in database. Creating new exercise.")
                    try:
                        existing_exercise = Exercise.objects.create(
                            name=exercise_name,
                            description={"description": f"Description for {exercise_name}"},
                            instructions={"setup": "Get into position", "execution": [], "form_tips": []},
                            exercise_type=exercise.get("exercise_type", "strength"),
                            muscle_group="",  # Update as needed
                        )
                        logger.info(f"New exercise '{exercise_name}' created with ID {existing_exercise.id}.")
                    except Exception as e:
                        logger.error(f"Error creating exercise '{exercise_name}': {e}")
                        continue

                # Fetch video details if videoId is not already present
                if not exercise.get('video_id'):
                    try:
                        video_data = get_youtube_video(exercise_name)
                        if video_data:
                            exercise['video_id'] = video_data['video_id']
                            exercise['thumbnail_url'] = video_data['thumbnail_url']
                            exercise['video_url'] = video_data['video_url']

                            # Update the database record as well
                            existing_exercise.videoId = video_data['video_id']
                            existing_exercise.thumbnail_url = video_data['thumbnail_url']
                            existing_exercise.video_url = video_data['video_url']
