...
const LogSessionForm = ({
  ...
  const getInitialFormValues = () => {
    ...
    if (currentWorkout?.exercises?.length > 0 && availableExercises.length > 0) {
  ...
  const validationSchema = Yup.object({
    ...
    exercises: Yup.array().of(
      Yup.object({
        ...
        // Relaxing the requirement for exercise_id to allow submission
      ...
      const sessionData = {
        ...
        exercises: values.exercises.map(exercise => {
          ...
          return {
            ...
            weight: (exercise.exercise_type === 'strength') ? exercise.weight : null,
  ...
  // If exercises are still loading, show a spinner
  if (exercisesLoading) {
    return (
      <Box textAlign="center" mt={4}>
        <CircularProgress />
        <Typography variant="body1" sx={{ mt: 2 }}>Loading exercises...</Typography>
      </Box>
    );
  }
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} md={4}>
                        <TextField
                          ...
                          onChange={formik.handleChange}
                          error={formik.touched.session_date && Boolean(formik.errors.session_date)}
                          helperText={formik.touched.session_date && formik.errors.session_date}
                          sx={{ opacity: 0.9 }}
                          InputLabelProps={{
                            shrink: true
                          }}
